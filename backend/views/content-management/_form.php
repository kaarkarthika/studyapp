...
<div class="box box-primary">
  <div class=" box-header with-border box-header-bg">
    <h3 class="box-title pull-left " >Chapter Management</h3>
  ...
  <div class="box-body min-height-box" >
    ...
    <div class="row">
      <div class="col-sm-4 form-group opt_check">
          <?php  echo $form->field($model, 'option_key')->dropDownList(['U' => 'Upload PDF', 'L' => 'Link'],['prompt'=>'Select Option']); ?>
      </div>
      <?php if($model->option_key=="U"){ ?>
        <div class="col-sm-4 form-group" id="upload_pdf">
         <?php
            $prevIng = $user_image_url = "";
            $no = "no";
            if($model->upload_file!=""){
                $no = "yes";
                $user_image_url = url::base(true)."/".$model->upload_file; 
            }
            if($user_image_url!=""){
     ...
     <label>Upload PDF</label>
          <input type="file" id="input-file-now-custom-1" class="dropify" name="ContentManagement[upload_file]" data-default-file="<?php echo $user_image_url; ?>" data-height="290" /><br />
      </div>
        <?php }else{ ?>
         <div class="col-sm-4 form-group" style="display:none;" id="upload_pdf">
         <?php
            $prevIng = $user_image_url = "";
            $no = "no";
            if($model->upload_file!=""){
     ...
     <label>Upload PDF</label>
       ...
       <?php }  ?>
       <?php if($model->option_key=="L"){ ?>
      <div class="col-sm-4 form-group" id="upload_link">
        <?= $form->field($model, 'upload_link')->textInput(['maxlength' => true]) ?>
      </div>
  <?php }else{ ?>
<div class="col-sm-4 form-group" style="display:none;" id="upload_link">
        <?= $form->field($model, 'upload_link')->textInput(['maxlength' => true]) ?>
      </div>
 <?php } ?>
...
<script>
$('body').on('click','.opt_check',function(){
      if($("#contentmanagement-option_key").val()== "U"){
         $('#upload_pdf').show();
         $('#upload_link').hide();   
        }else if($("#contentmanagement-option_key").val()== "L"){
         $('#upload_pdf').hide();
         $('#upload_link').show(); 
        }
  });
